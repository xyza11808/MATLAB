function varargout = Mannual_events_Adding(varargin)
...
%      singleton*.
%
%      H = MANNUAL_EVENTS_ADDING returns the handle to a new MANNUAL_EVENTS_ADDING or the handle to
%      the existing singleton*.
%
%      MANNUAL_EVENTS_ADDING('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in MANNUAL_EVENTS_ADDING.M with the given input arguments.
%
%      MANNUAL_EVENTS_ADDING('Property','Value',...) creates a new MANNUAL_EVENTS_ADDING or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before Mannual_events_Adding_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to Mannual_events_Adding_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help Mannual_events_Adding

% Last Modified by GUIDE v2.5 02-Oct-2020 21:03:48

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @Mannual_events_Adding_OpeningFcn, ...
                   'gui_OutputFcn',  @Mannual_events_Adding_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before Mannual_events_Adding is made visible.
function Mannual_events_Adding_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to Mannual_events_Adding (see VARARGIN)
global SessROISummary
if isempty(varargin) || isempty(varargin{1})
    warning('Need input ROI data for further processing.');
    return;
end
% SessROISummary.Path = '';
SessROISummary.ROIData = varargin{1};
SessROISummary.nROIs = size(SessROISummary.ROIData, 1);
SessROISummary.CurrentROI = 1;
set(handles.cROI_tag,'String','1');
set(handles.TotalROIString,'String',num2str(SessROISummary.nROIs));
SessROISummary.ROIEventsData = {};

if length(varargin) > 1 && isempty(varargin{2})
    % if exist events data is given
    set(handles.MessageTextTag,'String','Load existed Events data');
    SessROISummary.ModifiedEventData = varargin{2};
else
    SessROISummary.ModifiedEventData = cell(SessROISummary.nROIs, 1);
end
...
% Choose default command line output for Mannual_events_Adding
handles.output = hObject;

% Update handles structure
guidata(hObject, handles);

% UIWAIT makes Mannual_events_Adding wait for user response (see UIRESUME)
% uiwait(handles.Eventselectiontag);


% --- Outputs from this function are returned to the command line.
function varargout = Mannual_events_Adding_OutputFcn(hObject, eventdata, handles) 
...
if  InputValue < 1 || InputValue > SessROISummary.nROIs
    set(handles.cROI_tag,'string',num2str(SessROISummary.CurrentROI));
    return;
end

SessROISummary.CurrentROI = InputValue;

Plot_cROI_tag_Callback(hObject, eventdata, handles);
...
if ishandle(SessROISummary.ROIfig)
    ...
    set(handles.MessageTextTag,'String','No ROI trace was Plotted.');
    return;
end
[xx,~] = ginput;
...
cAxYlims = get(gca,'ylim');
if ~isempty(SessROISummary.AddEventEnd)
...
% --- Executes on button press in Plot_cROI_tag.
function Plot_cROI_tag_Callback(hObject, eventdata, handles)
% hObject    handle to Plot_cROI_tag (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
global SessROISummary
...
if ishandle(SessROISummary.ROIfig)
    ...
    hf = figure('position', [100 100 1500 400]);
    SessROISummary.ROIfig = hf;
end
hold on
plot(cROIRawData,'k','linewidth',1.2);
...
if isempty(SessROISummary.AddEventStart) || isempty(SessROISummary.AddEventEnd)
    ...
    SessROISummary.AddEventStart = [];
    SessROISummary.AddEventEnd = [];
    set(handles.MessageTextTag,'String','Event was added.00000000');
end
...
% handles    structure with handles and user data (see GUIDATA)
global SessROISummary
MannualEvents = SessROISummary.ModifiedEventData;
assignin('base','MannualEvents',MannualEvents);
assignin('base','SessDataSummary',SessROISummary);
closereq(); 
% delete(handles.Eventselectiontag);
% varargout = Mannual_events_Adding_OutputFcn(hObject, eventdata, handles);
