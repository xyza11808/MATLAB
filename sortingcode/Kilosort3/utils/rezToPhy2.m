...
[tempAmpsUnscaled,MaxChnInds] = max(tempChanAmps,[],2);
...
if ~isempty(savePath)
    ...
    writeNPY(MaxChnInds, fullfile(savePath, 'templates_maxChn.npy'));
    ...
    % Convert2infoFun(); % convert the putputs into cluster_info.tsv file, which usually output by phy
    save(fullfile(savePath, 'ClusterTypeANDchn.mat'),'ClusterTypes','ClusterBestChn','-v7.3');
