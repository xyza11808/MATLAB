...
%   Non-standard colormaps available within HEATMAP include 'money', 'red' and 'ddc'.
...
%   colormap is downsampled. If 'ddc' is selected as colormap,
%   'colorlevels' is forced equal to 5.
%
...
% * The 'ddc' colormap displays a colormap where 0 values are mapped to
%   yellow, negative values displayed in varying shades of blue and positive
%   values in varying shades of red
%
% EXAMPLES:
...
%         'TextColor', 'b')
% Copyright (c) 2014, Ameya Deoras
% All rights reserved.
%
% This function is slightly adapted from the original code by Ameya Deoras
% (available at MathWorks File Exchange) and is part of 
% LIBRA: the Matlab Library for Robust Analysis,
% available at:
...
if strcmp(param.Colormap, 'ddc')
    param.ColorLevels = 5;
    param.MinColorValue = 0;
    param.MaxColorValue = 4;
    param.GridLines='-';
    param.ShowAllTicks=true;
    
    param.ddc = 1;
    param.ddcblock = 0;
end

if strcmp(param.Colormap, 'ddcblock')
    param.ColorLevels = 5*param.ColorLevels;
    param.MinColorValue = 0;
    param.MaxColorValue = 5-eps;
    param.GridLines='-';
    param.ShowAllTicks=true;
...
if p.ExplicitlyComputeImage
    ...
    if p.ddcblock
        mat = 1 + mat * (p.ColorLevels/5); %scale mat from 0-5 to 1-320
        n = 1;
        x = p.ColorLevels;
        p.cdata = floor((mat-n)/(x-n)*(p.ColorLevels-1)+1);
...
if (p.ddc || p.ddcblock)
    set(p.hAxes, 'DataAspectRatio', [1 1 1]);
end
...
if ~strcmp(p.GridLines,'none')
    ...
    for i = 1:diff(xlim)-1
        line('Parent',p.hAxes,'XData',[i i]+.5, 'YData', ylim, 'LineStyle', p.GridLines,'Color','white');
    end
    for i = 1:diff(ylim)-1
        line('Parent',p.hAxes,'XData',xlim, 'YData', [i i]+.5, 'LineStyle', p.GridLines,'Color','white');
    end
end
end
...
if isempty(ylab) % No ticks or labels
    ...
    if (p.ddc||p.ddcblock); set(p.hAxes,'TickLength',[ 0 0 ]);end
end
if p.IsGraphics2
...
if ~isempty(p.yTitle)
   hylabel = get(p.hAxes,'ylabel');
   set(hylabel,'String',p.yTitle);
   set(hylabel,'FontWeight','bold');
   set(hylabel,'FontSize',p.TickFontSize);
end
...
if ~isempty(p.xTitle) %&& p.TickAngle == 0
   hxlabel = get(p.hAxes,'xlabel');
   set(hxlabel,'String',p.xTitle);
   set(hxlabel,'FontWeight','bold');
   set(hxlabel,'FontSize',p.TickFontSize);
   if ~isempty(hXText)

      %reserve space for xlabel
      set(hxlabel,'Units','normalized');
      extent=get(hxlabel,'Extent');labelH_N=extent(4);
      axPosN = get(p.hAxes,'Position'); 
      axPosN(2) = axPosN(2) + labelH_N; axPosN(4) = axPosN(4) - labelH_N;
      set(p.hAxes,'Position', axPosN) 
      
      %determine location of xlabel
	  set(hXText,'Units','data');
      set(hxlabel,'Units','data');
      xlabelPos=get(hxlabel,'Position');
      textExtent=get(hXText,'Extent');
      textExtent=cell2mat(textExtent);
      textMaxB=max(textExtent(:,2)); 
      
      set(hxlabel,'Position',[xlabelPos(1) textMaxB]);
   end
end

if ~isempty(p.Title)
   hTitle = get(p.hAxes,'Title');
   set(hTitle,'String',p.Title);
   set(hTitle,'FontWeight','bold');
   set(hTitle,'FontSize',p.TickFontSize*1.1);
end
...
[axPosP, axPosN, axOPP, ~, coPosP, textPosP]  = ...
...
axPosN = axPosN + [0 axPosN(4)-axHeightN+0.02 0 axHeightN-axPosN(4)]; %0.01 for xlabel
set(hAxes,'Position', axPosN) %[left bottom width height]
...
elseif iscellstr(textmat) && numel(textmat)==numel(mat) % If cell array of strings, it is already formatted
    ...
    error('textmat is incorrectly specified');
end
...
if p.ddc
    textColors = displaytext;
    textColors(mat==0) = {'black'};
    textColors(mat==1) = {'white'};
    textColors(mat==2) = {'white'};
    textColors(mat==3) = {'white'};
    textColors(mat==4) = {'black'};
    colorprop = 'Color';
    textLength = length(displaytext(:));
    p.hText = zeros(textLength,1);
    for i = 1:textLength
        if p.FontSize > 0
            p.hText(i) = text(xpos(i),ypos(i),displaytext{i},'FontSize',p.FontSize,...
                'HorizontalAlignment','center', colorprop, textColors{i},'Parent',p.hAxes);
        else
            p.hText(i) = text(xpos(i),ypos(i),displaytext{i},'Visible','off',...
                'HorizontalAlignment','center', colorprop, textColors{i},'Parent',p.hAxes);
        end
    end
else
...
elseif ischar(p.Colormap) || isa(p.Colormap,'function_handle')
    ...
    elseif strcmp(p.Colormap, 'ddc')
        p.Colormap = ddc(mat);
    elseif strcmp(p.Colormap, 'ddcblock')
        p.Colormap = ddcblock(p.ColorLevels);        
    else
...
% Generate ddc color map
function cmap = ddc(data)
...
% Generate ddc block color maps
function cmap = ddcblock(clevels)
    
    clevels = clevels/5; %divide in 5 subblocks

    code0 = repmat([1 1 0],clevels,1);
    ...
    r = interp1([1 clevels], [1 1], 1:clevels);
    g = interp1([1 clevels], [1 0], 1:clevels);
    b = interp1([1 clevels], [0 0], 1:clevels);
    ...
    code4 = repmat([1 1 1],clevels,1);
    
    cmap = [code0;code1;code2;code3;code4];
end
