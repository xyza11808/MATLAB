...
%               d = d12 or [d12 d13 d23]
    ...
    if (nOut==1 && ~vennOpts.Plot) || nOut==2
        if nCirc == 2
            ...
            catch % if there are no overlapping areas, only return the large component centers
                NewCentriod = moveCentroidOut(x,y,r);
%                 zoneCentroids = [[x(:), y(:)];nan(1,2)];
                zoneCentroids = [NewCentriod;nan(1,2)];
            end
        else
            try
                ...
                NewCentriod = moveCentroidOut(x,y,r);
%                 zoneCentroids = [[x(:), y(:)];nan(4,2)];
                zoneCentroids = [NewCentriod;nan(4,2)];
            end
        end
    end
...
function NewCentriod = moveCentroidOut(x,y,r)
% function used to move the centriod outward if the overlapping areas could
% not be calculated
if length(x) == 2
    x(1) = x(1) - r(1)*2/3;
    x(2) = x(2) + r(2)*2/3;
...
elseif length(x) == 3
    WholeVec = [x(:),y(:)];
    NewCentriod = zeros(3,2);
    for j = 1 : 3
        CenterPoint = [x(j),y(j)];
        Outvec = WholeVec;
        Outvec(j,:) = [];
        Outvec = Outvec - CenterPoint;
        Outvec_unit = Outvec;
        Outvec_unit(1,:) = Outvec_unit(1,:)/norm(Outvec_unit(1,:));
        Outvec_unit(2,:) = Outvec_unit(2,:)/norm(Outvec_unit(2,:));
        MoveOutLength = mean(Outvec_unit)*r(j)*2/3;
        NewCentriod(j,:) = CenterPoint - MoveOutLength;
    end
end
end