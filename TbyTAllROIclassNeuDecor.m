function varargout = TbyTAllROIclassNeuDecor(RawDataAll,StimAll,TrialResult,AlignFrame,FrameRate,varargin)
...
shufDataUsedAll = zeros(size(UsingData));
for nns = 1 : length(UsingStimType)
    cStimFreqInds = UsingStim == UsingStimType(nns);
    cStimData = UsingData(cStimFreqInds,:);
    cStimShfuData = zeros(size(cStimData));
    for nnR = 1 : size(cStimShfuData,2)
        cVector = cStimShfuData(:,nnR);
        cShufV = Vshuffle(cVector);
        cStimShfuData(:,nnR) = cShufV;
    end
    shufDataUsedAll(cStimFreqInds,:) = cStimShfuData;
end
UsingData = shufDataUsedAll;
