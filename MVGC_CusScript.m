...
alpha     = 0.025;   % significance level for significance test
mhtc      = 'FDR';  % multiple hypothesis test correction (see routine 'significance')

seed      = 0;      % random seed (0 for unseeded)

%% rescale ROI data scales to [0,1]

nROIs = size(X,1);
NormX = zeros(size(X));
for cR = 1 : nROIs
    cRTrace = X(cR,:);
    NormX(cR,:) = (cRTrace - min(cRTrace))/(max(cRTrace) - min(cRTrace));
end
...
% AT = var5_test;
nvars = size(NormX,1); % number of variables
nobs = size(NormX,2);
...
ptic('\n*** tsdata_to_infocrit\n');
[AIC,BIC] = tsdata_to_infocrit(NormX,momax,icregmode);
...
[F,A,SIG] = GCCA_tsdata_to_pwcgc(NormX,morder,regmode); % use same model order for reduced as for full regressions
ptoc;
...
pval = mvgc_pval(F,morder,nobs,1,1,1,nvars-2,tstat);
