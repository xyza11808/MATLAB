...
% example 3 for my data
% cR = 2;
% cRTrace = DataTrace(cR, :);
% CenterDistriDatas = cRTrace(cRTrace > prctile(cRTrace, 10) & cRTrace < prctile(cRTrace, 90));
% base = prctile(cRTrace, 40);
% Thres = base + std(CenterDistriDatas) * 3;
% [pks, locs] = findpeaks(cRTrace, 'MinPeakHeight', Thres, 'MinPeakWidth', 30);
% peakLoc = peakfinder(cRTrace, std(CenterDistriDatas) * 3, Thres);
% hf = figure('position', [2000 500 1700 420]);
% hold on
% plot(cRTrace,'k');
% line([1,numel(cRTrace)], [Thres Thres], 'Color', 'c','linestyle','--');
% hl1 = plot(locs, cRTrace(locs), 'ro', 'Markersize', 8,'linewidth',1);
% hl2 = plot(peakLoc, cRTrace(peakLoc), 'co', 'Markersize', 12,'linewidth',1);
% legend([hl1, hl2], {'Matlab', 'NewMethod'},'location','NorthEast','box','off');
