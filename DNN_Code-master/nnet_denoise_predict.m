function [lsizeOUT,ltypeOUT] = nnet_denoise_predict(alldata,trainIND,testIND,runName,noiseIters,sig2noise,predInd)
...
permInd=randperm(size(alldata,2)-predInd);

indata= alldata(:,permInd(1:trainIND));
outdata=alldata(:,permInd(1:trainIND)+predInd);
intest=alldata(:,permInd(testIND:end));
outtest=alldata(:,permInd(testIND:end)+predInd);
