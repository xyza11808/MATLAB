...
c_save = zeros(data_size(2),length(DB_type),length(freq_type));
...
ROIRFClim = zeros(data_size(2),2);
% ROICdata = cell(data_size(2),1);
...
elseif nargout > 1
    varargout(1) = {cf};
    vShapeDataStrc.RFclim = ROIRFClim;
    vShapeDataStrc.RFdataAll = c_save;
    vShapeDataStrc.FreqTypes = freq_type;
    vShapeDataStrc.DBTypes = DB_type;
    varargout(2) = {vShapeDataStrc};
end
if ~isplot
    fprintf('Plot is skipped in current analysis, quit function.\n');
    return;
end
%%
