function [im_dft, shift] = dft_reg_gpu(im_s, im_tg, pix_range)
...
im_cpu_s=im_s;
im_s=gpuArray(double(im_s));
im_tg=gpuArray(double(im_tg));
row_nums=gpuArray(row_nums);
col_nums=gpuArray(col_nums);
t_fft=tic;
for i=1:size(im_s,3);
%     if i <= span
...
%         imagesc(im_tg,[0 100]);   pause(0.2)
      [output(:,i), fft_frame_reg] = dftregistration(fft2(im_tg(row_nums,col_nums)),fft2(im_s(row_nums, col_nums,i)),1);
...
shift = output(3:4,:);
padding = [0 0 0 0];
im_dft = ImageTranslation_nx(im_cpu_s,shift,padding,0);