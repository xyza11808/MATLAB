function varargout = nx_imAnalysisGUI2013(varargin)
...
%      singleton*.
%
%      H = NX_IMANALYSISGUI2013 returns the handle to a new NX_IMANALYSISGUI2013 or the handle to
%      the existing singleton*.
%
%      NX_IMANALYSISGUI2013('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in NX_IMANALYSISGUI2013.M with the given input arguments.
%
%      NX_IMANALYSISGUI2013('Property','Value',...) creates a new NX_IMANALYSISGUI2013 or raises the
% nx_imAnalysisGUI2013 M-file for nx_imAnalysisGUI2013.fig
%      nx_imAnalysisGUI2013, by itself, creates a new nx_imAnalysisGUI2013 or raises the existing
%      singleton*.
%
...
%      applied to the GUI before nx_imAnalysisGUI2013_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to nx_imAnalysisGUI2013_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help nx_imAnalysisGUI2013
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @nx_imAnalysisGUI2013_OpeningFcn, ...
                   'gui_OutputFcn',  @nx_imAnalysisGUI2013_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before nx_imAnalysisGUI2013 is made visible.
function nx_imAnalysisGUI2013_OpeningFcn(hObject, eventdata, handles, varargin)
global CaSignal % ROIinfo ICA_ROIs
% Choose default command line output for nx_imAnalysisGUI2013
...
% UIWAIT makes nx_imAnalysisGUI2013 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = nx_imAnalysisGUI2013_OutputFcn(hObject, eventdata, handles) 
...
CaSignal.results_path = [datapath filesep 'Analysis_Results'];
...
if get(handles.dispCorrMapTrials,'Value')==1
    if isempty(CaSignal.CorrMapTrials) || ~isequal(CaSignal.CorrMapROINo, currROI)
        ...
        CaSignal.CorrMapTrials = load_scim_data([pathname filesep fn]);
