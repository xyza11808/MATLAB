...
%      singleton*.
%
%      H = CaSignal_ROI_GUI_par returns the handle to a new CaSignal_ROI_GUI_par or the handle to
%      the existing singleton*.
%
%      CaSignal_ROI_GUI_par('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in CaSignal_ROI_GUI_par.M with the given input arguments.
%
%      CaSignal_ROI_GUI_par('Property','Value',...) creates a new CaSignal_ROI_GUI_par or raises the
% CaSignal_ROI_GUI_par M-file for CaSignal_ROI_GUI_par.fig
%      CaSignal_ROI_GUI_par, by itself, creates a new CaSignal_ROI_GUI_par or raises the existing
%      singleton*.
%
...
%      applied to the GUI before CaSignal_ROI_GUI_par_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to CaSignal_ROI_GUI_par_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help CaSignal_ROI_GUI_par

% Last Modified by GUIDE v2.5 21-Oct-2015 11:24:04
...
gui_State = struct('gui_Name',       mfilename, ...
                   ...
                   'gui_OpeningFcn', @CaSignal_ROI_GUI_par_OpeningFcn, ...
                   'gui_OutputFcn',  @CaSignal_ROI_GUI_par_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before CaSignal_ROI_GUI_par is made visible.
function CaSignal_ROI_GUI_par_OpeningFcn(hObject, eventdata, handles, varargin)
clearvars -global
global CaSignal % ROIinfo ICA_ROIs
% Choose default command line output for CaSignal_ROI_GUI_par
...
% UIWAIT makes CaSignal_ROI_GUI_par wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = CaSignal_ROI_GUI_par_OutputFcn(hObject, eventdata, handles) 
...
global CaSignal % ROIinfo ICA_ROIs
% CaSignal_ROI_GUI_par_OpeningFcn(hObject, eventdata, handles);
...
if get(handles.dispMeanMode, 'Value')==1
    if ~isfield(CaSignal, 'h_mean_fig') || ~ishandle(CaSignal.h_mean_fig)
        CaSignal.h_mean_fig = figure('Name','Mean Image','Position',[30   60   1000   900]);
    else
...
if get(handles.dispMaxDelta,'Value')==1
    if ~isfield(CaSignal, 'h_maxDelta_fig') || ~ishandle(CaSignal.h_maxDelta_fig)
        CaSignal.h_maxDelta_fig = figure('Name','max Delta Image','Position',[30   60   1000   900]);
    else
...
if get(handles.dispMaxMode,'Value')==1
    if ~isfield(CaSignal, 'h_max_fig') || ~ishandle(CaSignal.h_max_fig)
        CaSignal.h_max_fig = figure('Name','Max Projection Image','Position',[30   60   1000   900]);
    else
...
%             ROI_updated_flag = 1;
        case'Re-draw'
            delete(h_roi);
            h_roi = feval(DrawROI); finish_drawing = 0;
        case'Cancel',
...
[F, dff] = extract_roi_fluo(im, CaSignal.ROIinfo(TrialNo), opt_subBG);
...
function [F, dff] = extract_roi_fluo(im, ROIinfo, opt_subBG)
...
for TrialNo = Start_trial:End_trial
    ...
    if  ~isempty(ROIpos)
        CaSignal.ROIinfo(TrialNo) = CaSignal.ROIinfo(1);
...
%     CalculatePlotButton_Callback(handles.figure1, eventdata, handles, im, ROIinfo_local(TrialNo), 0);
    [F, dff] = extract_roi_fluo(im, ROIinfo_local(TrialNo), 0);
