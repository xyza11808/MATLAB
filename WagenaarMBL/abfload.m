...
% ABFLOAD - Loads and returns data in the Axon abf format.
%    Data may have been acquired in the following modes:
%    (1) event-driven variable-length
%    (2) event-driven fixed-length 
%    (3) gap-free
%    Information about scaling, the time base and the number of channels
%    and episodes is extracted from the header of the abf file (see also
%    abfinfo.m). All optional input parameters listed below (= all except
%    the file name) must be specified as parameter/value pairs, e.g. as in 
%
...
%    start     scalar, 0          Only gap-free-data: start of cutout to 
%                                   be read (unit: sec).
%    stop      scalar or char,    Only gap-free-data: end of cutout to 
%                'e'                be read (unit: sec). 
%                                   May be set to 'e' (end of file).
%    sweeps    1d-array or char,  Only episodic data: sweep numbers to be
%                'a'                read. By default, all sweeps will be
%                                   read ('a').
%    channels  cell array         Names of channels to be read, like 
%                or char, 'a'       {'IN 0','IN 8'}.
%                                   Make sure spelling is 100% correct 
%                                   (including blanks).
...
%                                   (when less channels shall be read than 
%                                   exist in file).
%    machineF  char array,        The 'machineformat' input parameter of the
%               'ieee-le'           matlab fopen function. 'ieee-le' is 
%                                   the correct option for windows; depending 
%                                   on the platform the data were 
%                                   recorded/shall be read by abfload 
%                                   'ieee-be' is the alternative.
%    
%    OUTPUT VARIABLES
%    
%    NAME  TYPE            DESCRIPTION
%
%    d                     The data read, the format depending on
%                            the recording mode:
%          1. GAP-FREE:    2D array of size TxC
%                            Examples of access:
%                               d(:,2)      All data from channel 2.
%                               d(1:100,:)  First 100 data points 
%                                             from all channels
%          2. EPISODIC FIXED-LENGTH:
%                          3D array of size TxCxS
%                            Examples of access:
%                               d(:,2,:)  A matrix which contains all
%                                           events (at full length) 
%                                           of channel 2 in its columns.
%                                d(1:200,:,[1 11])
%                                         Contains first 200 data points
%                                           of events #1 and #11 of all
%                                           channels.
%          3. EPISODIC VARIABLE-LENGTH:
%                          Cell array whose elements correspond to 
%                            single sweeps. Each element is a (regular) 
%                            array of size TxC.
%                            Examples of access:
%                               d{1}  A 2D array which contains sweep #1
%                                       (all of it, all channels).
%                               d{2}(1:100,2)
%                                     A 1D array containing first 100 data 
%                                       points of of channel 2 in sweep #1.
%    
