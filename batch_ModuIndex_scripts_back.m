...
%     set(gca,'xlim',[-0.05,0.55],'xtick',0:0.1:0.5);
    ...
    for croi = 1 : length(PassMaxRespInds)
        ...
        if min(cPassData) < 0 || min(cTaskData) < 0
            ...
            PassMaxTaskValue(croi) = cTaskData(PassMaxRespInds(croi)) + cIncreaseLevel;
        else
...
figure;plot(DisDataAll,ModuIndexAll,'o')
hold on
[tbl,fitData] = lmFunCalPlot(DisDataAll,ModuIndexAll,0);
plot(fitData{1},fitData{2},'r','linewidth',1.4)