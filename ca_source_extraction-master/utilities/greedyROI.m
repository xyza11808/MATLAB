...
Y = bsxfun(@minus, Y, med); %substract frame median value from raw data
...
for r = 1:length(K)
    ...
    for k = 1:K(r),    
        ...
        iSig = [max(iHat - gHalf,1)', min(iHat + gHalf,dx)']; iSigLen = iSig(:,2) - iSig(:,1) + 1;
            %iSig is the current center sample box, and iSigLen is the
            %sample box size
        %fine tune the shape
        if dimY == 2
            dataTemp = Y(iSig(1,1):iSig(1,2), iSig(2,1):iSig(2,2), :);  %sample box data
            traceTemp = squeeze(rho(iHat(1), iHat(2), :)); %sample center time data
        else
        ...
        if k < K(r)
            ...
            dataTemp = imblur(dataTemp, gSig, gSiz, dimY, 0);
...
res = reshape(Y,d,T) + repmat(med(:),1,T);  %real residuals for raw data
[b_in,f_in] = nnmf(max(res,0),nb);  %max(res,0) is to set all negtive value into 0, and nnmf will seperate resduals explanation into spatial
%and temporal component by Nonnegative matrix factorization method
...
function [ain,cin] = finetune(data,cin,nIter)
    ...
    data = reshape(data,prod(iSigLen),T);  %each box to row, and time to column
    for iter = 1:nIter
        %nc = norm(cin)^2;
