...
Yres = reshape(Y, prod(d), []) - A*C - b*f;  %Raw data substract of data component and residues
...
for miter=1:maxIter
    for mcell = 1:K
        ...
        c0 = C(mcell, :);  %current ROI trace
        a0 = A(ind_pixels, mcell); %current ROI mask
        norm_a2 = norm(a0, 2)^2;   %square sum for vector
