function FieldDatas_AllCell = IsFieldDataPath(InputPath)
        ...
        for cf_field = 1 : NumFields
            ...
            try
                ...
                for cFieldSess = 1 : C_Field_ExpNumbers
                    MoveFreeTraceRaw_All{cFieldSess} = SimpleROIExtraction(...
                        fullfile(cField_FullPaths{cFieldSess},'Aligned_datas'),...
                        FullROIInfoDatas);
                end
                %%
                ...
                MoveFreedff_All_Mtx = (MoveFreeTraceRaw_Mtx - MoveFreePrcBaseData)./...
                    MoveFreePrcBaseData;
                FieldDatas_AllCell{cf_field,1} = MoveFreedff_All_Mtx;
...
%                 for cROI = 1 : nROIs
%                     cROITrace = MoveFreedff_All_Mtx(cROI,:);
...
%                 CorrCoefMtx = corrcoef(MoveFreedff_All_Mtx');
...
% 
%                     saveas(hf,sprintf('Cutoff value %02d correlation mtx plot',CuroffValues(c_Cut)*10));
%                     saveas(hf,sprintf('Cutoff value %02d correlation mtx plot',CuroffValues(c_Cut)*10),'png');
%                     close(hf);
%                 end
...
%                 saveas(hhf,'Coef vs Distance plots save');
%                 saveas(hhf,'Coef vs Distance plots save','png');
%                 close(hhf);
% 
%                 save CorrCoefData.mat CorrCoefMtx ROICenterPosMtx ROIdis_mtx ROI_Dis_coefData BincentANDdatas ROIdataStrc -v7.3
