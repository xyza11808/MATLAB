...
for cfff = 1:NumFolders
    ...
    SessionFieldData =  CorrDataProcessFun(cfFullPath);
...
title('p14 old ')
%%
saveas(hf,'All pair noise correlation changes');
saveas(hf,'All pair noise correlation changes','png');
saveas(hf,'All pair noise correlation changes','pdf');
...
xlabel('Distance (um)');
ylabel('Noise coefficient');
title('p14 old ActAst\_AllNeu')
%%
saveas(hf,'ActAst_AllNeu pair noise correlation changes');
saveas(hf,'ActAst_AllNeu pair noise correlation changes','png');
saveas(hf,'ActAst_AllNeu pair noise correlation changes','pdf');
...
ylabel('Fraction');
title(sprintf('p14 WT-Tg-Neu-p = %.2e',WT_Tg_Neu_p));
text(10,0.2,sprintf('WT-Tg-Ast p %.2e',WT_Tg_Ast_p),'HorizontalAlignment','center');
%%
saveas(hEventfreq,'Events frequency p14 plots save');
saveas(hEventfreq,'Events frequency p14 plots save','png');
saveas(hEventfreq,'Events frequency p14 plots save','pdf');

%% event synchrony analysis
% EventSyncFrac
WTSessInds = strcmpi('wt',MultiSessGenoType);
WTEventIndexData_cell = EventSyncFrac(WTSessInds);
WTEventIndexData_FieldCell = cat(1,WTEventIndexData_cell{:});

TgEventIndexData_cell = EventSyncFrac(~WTSessInds);
...
ylabel('PeakFrac');
title('p14 Peak sync fraction');
%%
saveas(HHf,'p14 Sync Peak number and fraction compare plot');
saveas(HHf,'p14 Sync Peak number and fraction compare plot','png');
saveas(HHf,'p14 Sync Peak number and fraction compare plot','pdf');
...
% RealIndex = smooth(RealIndex,60);
[Peaks,locs] = findpeaks(RealIndex,'MinPeakDistance',270,'MinPeakHeight',BaseValue*1.01);
