function h = plot_scattered(pos,z, resc)
...
if size(pos, 1)==3
    ...
    if ~isempty(z)
        h = scatter3(pos(1,:),pos(2,:),pos(3,:),12,z,'filled');
    else
        h = scatter3(pos(1,:),pos(2,:),pos(3,:),12,pos(1,:),'filled');
    end
...
if nargin<3
    if ~isempty(z)
        h = scatter(pos(1,:),pos(2,:),12,z,'filled');
    else
        h = scatter(pos(1,:),pos(2,:),20,'filled');
    end
    % plot( pos(1,:), pos(2,:), '.' );    
%    axis equal;
    axis tight;
