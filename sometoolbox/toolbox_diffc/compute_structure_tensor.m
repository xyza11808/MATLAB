...
%   U. Kothe, "Edge and Junction Detection with an Improved Structure Tensor", 
...
options.null = 0;
sub = getoptions(options, 'sub', 2);
use_anisotropic = getoptions(options, 'use_anisotropic', 0);
use_grad_renormalization = getoptions(options, 'use_grad_renormalization', 0);
use_tensor_renormalization = getoptions(options, 'use_tensor_renormalization', 0);
m_theta = getoptions(options, 'm_theta', 16);
sigmat = getoptions(options, 'sigmat', .4);
...
% smooth
if ~use_anisotropic
    m = round( sub*sigma2*1.2 )*2+1;
    m = min(m, round(n/2)*2+1);
