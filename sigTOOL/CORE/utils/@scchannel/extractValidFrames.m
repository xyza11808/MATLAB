function [data tb epochs trig]=extractValidFrames(channel, trig, duration, pretime)
% extractValidFrames extracts framed adc data from valid epochs
%
% Example:
% [data tb epochs trig]=extractValidFrames(channel, trig, duration, pretime)
% where
...
% Get the indices
[idx epochs trig]=findValidFrameIndices(channel, trig, duration, pretime);
if isempty(idx)
    % No valid frames
    data=[];
    tb=[];
