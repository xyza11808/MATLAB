function varargout=RandNMTChoiceDecoding_RF(RawDataAll,BehavStrc,TrialResult,AlignFrame,FrameRate,varargin)
...
BehavStims = double(boundary_result.StimType(:));
nPairs = floor(length(BehavStims)/2);
OctaveRealx=log2(double(BehavStims)/min(double(BehavStims)));
OctavePassx=log2(double(Freqtypes)/8000);  % passive frequency types
% Octavefit=Octavex;
Octavexfit=OctavePassx;
% OctaveTest=Octavex;
realy=boundary_result.StimCorr;
realy(1:nPairs)=1-realy(1:nPairs);
% Curve_x=linspace(min(OctaveRealx),max(OctaveRealx),500);
rescaleB=max(realy);
rescaleA=min(realy);
...
scatter(OctaveRealx,realy,80,'k','o','LineWidth',2);
...
set(gca,'xtick',OctaveRealx,'xticklabel',cellstr(num2str(CorrStimTypeTick(:),'%.2f')),'FontSize',20);
set(gca,'FontSize',18);
if length(TimeLength) == 1
...
save NMDataSummry.mat Octavexfit OctaveRealx realy fityAll BehavRestult NeuFitResult -v7.3
