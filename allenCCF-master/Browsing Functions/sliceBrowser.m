...
function ud = updateSliceImage(ud)
    ...
    current_slice_image = flip(im2uint8(imread(fullfile(ud.processed_images_folder, processed_image_name))),1);
...
%     ud.im.Parent = [0 256];
%     UsedClims = prctile(double(current_slice_image(:)),[55,99])+[-1000, 1000];
    set(ud.im.Parent,'Clim',[0 256]);
