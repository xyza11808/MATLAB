...
%     return;
% end
RespUnitfile = fullfile(savefolder,'UnitSelectiveTypes2.mat');
RespUnitMtx = load(RespUnitfile,'IsUnitGLMResp');
RespUnitIndex = find(sum(RespUnitMtx.IsUnitGLMResp,2)>0);
NumRespUnits = length(RespUnitIndex);
...
AllTaskEventsFull = [StimEventshiftMtxs_Used,Choice1EventshiftMtxs_Used,Choice2EventshiftMtxs_Used,...
    BTEventshiftMtxs_Used,TrIndexMtxsNew_Used]; 
...
EventDescripStrsFull = {'Stim','ChoiceL','ChoiceR','BTBinary','TrIndex'};
...
%% BinnedSPdatas
tic
RegressorResidues = cell(NumRespUnits,3);
% f = waitbar(0,'Session Calculation Start...');
% only significant resp units will be calculated

ErrorU = zeros(NumRespUnits,1);
for cU = 1: NumRespUnits %NumNeurons
    try
        [PredANDResidueDataCell, Eventmodel_explain_var, EventResidue] = ...
            lassoRegressor_residueCal(BinnedSPdatas(cU,:), FullEvents_predictor, 5);
        RegressorResidues(cU,:) = {PredANDResidueDataCell, Eventmodel_explain_var, EventResidue};
...
% close(f);
%%
save(dataSaveNames, 'RegressorResidues','RespUnitIndex',...
    'ExistField_ClusIDs', 'NewAdd_ExistAreaNames', 'AreaUnitNumbers',...
    'EventDescripStrsFull','FullEvents_predictor','-v7.3');
