function [P_L,nll_fit,delta,V_R,V_L,P_bound_low,P_bound_high]= MB_RL_function_v3(theta_pred,beta_pred,alpha_pred,Plow_init,Octave_used,left_choice_used,inds_correct_used)
...
for t = 1:length(Octave_used)
    ...
    if (P_low_1(t) ==0 && P_low_2(t) == 0) || (P_low_1(t) ==1 && P_low_2(t) == 1)
        p_HighB_givenA(t) = 0.5;
        p_LowB_givenA(t) = 0.5;
    end
    
    delta(t) =  inds_correct_used(t) - V_Chosen(t);
    
%     V_update(t) = V_Chosen(t) + alpha_pred*delta(t);
    if left_choice_used(t)
       ...
       P_bound_low(t+1) = P_bound_low(t) + alpha_pred*delta(t)*V_Chosen_low(t);%*p_LowB_givenA(t);
