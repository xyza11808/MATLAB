...
%     regressors_gpu = ([[regressor_design,constant];ridge_matrix]);
%     signals_gpu = ([signals';zeros(length(ridge_matrix),size(signals,1))]);
end
...
predicted_signals = nan(size(signals),'single');
predicted_signals_reduced = nan(size(signals,1),size(signals,2),length(regressors),'single');
predicted_signals_partial = nan(size(signals,1),size(signals,2),length(regressors),2,'single');
