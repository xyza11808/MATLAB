...
Choices_Mtx = Trial_Choices;
Choices_Mtx(Trial_Choices == 0) = -1;
ReParas_mtx = double(ReTimes > 0);
BlockTypes = TrialBlockTypes;
BlockTypes(BlockTypes == 0) = -1;
...
AfterStimwinbin = round([0,1500]/1000/ProbNPSess.USedbin(2));
AnsRe_winbin = round([0,2000]/1000/ProbNPSess.USedbin(2));
...
elseif strcmpi(ProbNPSess.TrigAlignType{ProbNPSess.CurrentSessInds},'stim') 
    ...
    StimResp1 = mean(SMBinDataMtx(:,:,(TriggerAlignBin+Stimwinbin(1,1)+1):(TriggerAlignBin+Stimwinbin(1,2))),3);%;
%     StimResp2 = mean(SMBinDataMtx(:,:,(TriggerAlignBin+Stimwinbin(2,1)+1):(TriggerAlignBin+Stimwinbin(2,2))),3);% - BaselineResp;
    ShufStimResp1 = mean(TShiftSMBinDataMtx(:,:,(TriggerAlignBin+Stimwinbin(1,1)+1):(TriggerAlignBin+Stimwinbin(1,2))),3);%;
...
% AllParas_mtx = zeros(TrNum,NumFreqTypes+2+1);%+2
AllParas_mtx = [FreqParas_mtx, Choices_Mtx, ReParas_mtx, BlockTypes];
...
parfor cUnit = 1 : unitNum
    warning off
    cUnitRespVec = StimResp1(:,cUnit);
    cUnitRespVec_shuf = ShufStimResp1(:,cUnit);
...
%     cUnitRespVec = cUnitRespVec/mean(cUnitRespVec);
    ...
    for cRepeat = 1 : NumRepeats
        ...
        md2 = fitglm(AllParas_mtx(trainInds,:),cUnitRespVec(trainInds,:),overAllTerms_mtx,'CategoricalVars',...
            logical([ones(1,NumFreqTypes),0,0,0]));
