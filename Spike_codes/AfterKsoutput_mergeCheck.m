function [FinalSPClusters,FinalSPTimeSample,FinalSPMaxChn,FinalTemplateUW,FinalksLabels] = ...
    AfterKsoutput_mergeCheck(FolderPath)
% cclr

% FolderPath = pwd;
% load(fullfile(FolderPath,'NPClassHandleSaved.mat'));
mergelogfilepath = fullfile(FolderPath,'ClusterMergeLog.txt');
fid = fopen(mergelogfilepath,'w');
fclose(fid);
%%
...
TrigTimeANDSampleNumStrc = load(fullfile(FolderPath,'TrigTimeANDallSampNum.mat'));
TotalSampleTime = TrigTimeANDSampleNumStrc.TotalSampleNums/SpikeStrc.sample_rate;
...
[ClusNeedCheckInds,GoodClusWaveShape,GoodClusMaxChn,GoodClusIDs,GoodClusFRs,~,ClusTypesAll] = locateNeedCheckClus(SpikeClus,...
...
%
LogFileID = fopen(mergelogfilepath,'a+');
fprintf(LogFileID,'###################################################\n');
fprintf(LogFileID,'Operation log for first time merge:\n');
for cMer = 1 : CurrentMergeTimes
    fprintf(LogFileID,sprintf('Merge operation %d:\n',cMer));
    ...
    MergelogStr = cell(1,NumMergeClus);
%     sampleShifts = cell(1,NumMergeClus);
    for cMergClus = 1 : NumMergeClus
        ...
        AfterMergeSpikeTimeSample(SpikeClus == cMerClus_IDs(cMergClus)) = SpikeTimeSample(SpikeClus == cMerClus_IDs(cMergClus))...
            - lags(maxinds);
%         sampleShifts{cMergClus} = sprintf('  %d  ', - lags(maxinds));
        MergelogStr{cMergClus} = sprintf('  %d(%d)  ',cMerClus_IDs(cMergClus),-lags(maxinds));
...
%         AfterMergeSpikeAmps(SpikeClus == cMerClus_IDs(cMergClus)) = SpikeAmplitude(SpikeClus == cMerClus_IDs(cMergClus))/ChangeRatio;
    ...
    AllMergeIDStrs = cat(2,MergelogStr{:});
    formatSpec = 'Merge clusterIDs%sinto New cluster ID %d.\n';
    fprintf(LogFileID,formatSpec,AllMergeIDStrs,NewClusIDs);
...
fprintf(LogFileID,'Merge operation after first check period is ended.\n');
fclose(LogFileID);
%% calculate the cluster types after first time merge
[AM1NeedCheckInds,AM1GoodClusWaveShape,AM1GoodClusMaxChn,AM1GoodClus,AM1GoodClusFRs,~,AM1ClusTypes] = ...
...
LogFileID = fopen(mergelogfilepath,'a+');
fprintf(LogFileID,'\n###################################################\n');
fprintf(LogFileID,'Operation log for second time merge:\n');
...
fprintf(LogFileID,'Merge operation after second check period is ended.\n');
fclose(LogFileID);
%% third time check
[AM2NeedCheckInds,AM2GoodClusWaveShape,AM2GoodClusMaxChn,AM2GoodClus,AM2GoodClusFRs,~,AM2ClusTypes] = ...
...
LogFileID = fopen(mergelogfilepath,'a+');
fprintf(LogFileID,'\n###################################################\n');
fprintf(LogFileID,'Operation log for third time merge:\n');
...
for cMer = 1 : CurrentMergeTimes
    ...
    disp(cMerClus_IDs');
    fprintf('Into new cluster %d.\n',NewClusIDs);
    MergelogStr = cell(NumMergeClus,1);
    for cMergClus = 1 : NumMergeClus
...
fprintf(LogFileID,'Merge operation after third check period is ended.\n');
fclose(LogFileID);
