function [fityAll,bfit]=RandNeuroBatch(RawDataAll,StimAll,TrialResult,AlignFrame,FrameRate,Octavex,boundary_result,varargin)
...
if nargin>8
    if ~isempty(varargin{2})
        isLoadModelInput = varargin{2};
        isLoadModel = isLoadModelInput.IsModelLoad;
    end
end
...
if isLoadModel
    ExCoef = isLoadModelInput.ExtraCoef;
    Exmodel = isLoadModelInput.ExtraModel;
    NorScale = isLoadModelInput.NormalScale;
end
...
if ~isLoadModel
    if abs(max(difscore)) > 2*abs(min(difscore))
        fityAll=(rescaleB-rescaleA)*((difscore-min(difscore))./(abs(min(difscore))-min(difscore)))+rescaleA; %rescale to min and max behav performance
        fityAll(fityAll>rescaleB) = rescaleB;
        NorScaleValue = [abs(min(difscore)),min(difscore)];
    elseif abs(min(difscore)) > 2 * abs(max(difscore))
        fityAll=(rescaleB-rescaleA)*((difscore+max(difscore))./abs((max(difscore)*2)))+rescaleA; %rescale to min and max behav performance
        fityAll(fityAll<rescaleA) = rescaleA;
        NorScaleValue = [abs(max(difscore)),max(difscore)];
    else
        fityAll=(rescaleB-rescaleA)*((difscore-min(difscore))./(max(difscore)-min(difscore)))+rescaleA;  %rescale to min and max behav performance
        NorScaleValue = [max(difscore),min(difscore)];
    end
else
    NorScaleValue = NorScale;
