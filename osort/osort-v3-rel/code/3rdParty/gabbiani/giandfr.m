function  [sys, x0, str, ts]  = giandfr(t,x,u,flag,dt,thres,n,C,ref)
%giandfr: S-function that implements an integrate-and-fire neuron
%model with gamma distributed random threshold and refractory period. 
...
%	randomthres_i+1 = random number drawn from a gamma
%	                  distribution with mean=thres and order n, if
%	                  Vm_i >= randomthres_i
%	
...
if (abs(flag) == 2)    	% Discrete state update
  if ( x(1) >= x(3) )	% threshold reached, next voltage value is reset
    ...
    sys(3) = gamrnd(n,thres/n); %updates the threshold
...
elseif ( flag  == 0 )	% Initialization
  ...
  x0 = [0, 0, gamrnd(n,thres/n)];	% Initialization of the membrane voltage,
