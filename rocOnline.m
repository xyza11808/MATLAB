function [ROCout,LabelMeanS]=rocOnline(varargin)
...
% tr=repmat('-',1,100);
lu=length(x(x(:,2)==InputLabelType(1))); %number of unhealthy subjects
lh=length(x(x(:,2)==InputLabelType(2))); %number of healthy subjects
z=sortrows(x,1);
if threshold==0
...
if length(labels)>=400
    percent=linspace(1,100,300);
    labels=prctile(labels,percent);
%     [~,nCenters]=hist(labels,300);
%     labels=nCenters;
end
labels(end+1)=labels(end)+1;
ll=length(labels); %count unique value
a=zeros(ll,2); b=a; c=zeros(ll,1);%array preallocation
ubar=mean(x(x(:,2)==InputLabelType(1)),1); %unhealthy mean value
hbar=mean(x(x(:,2)==InputLabelType(2)),1); %healthy mean value
LabelMeanS.LabelType1=InputLabelType(1);
LabelMeanS.LabelType2=InputLabelType(2);
LabelMeanS.Type1value=ubar;
LabelMeanS.Type2value=hbar;
Type0data=x(x(:,2)==InputLabelType(1));
Type1data=x(x(:,2)==InputLabelType(2));
[N0,C0]=hist(Type0data,100);
[N1,C1]=hist(Type1data,100);
H=figure('position',[100 110 1700 960],'PaperPositionMode','auto');
subplot(1,2,1);
hold on;
plot(C0,N0,'b-o','LineWidth',0.9);
plot(C1,N1,'r-o','LineWidth',0.9);
legend('Label 0 data','Label 1 data');

for K=1:ll
    if hbar<ubar
        TP=length(x(x(:,2)==InputLabelType(1) & x(:,1)>labels(K)));
        FP=length(x(x(:,2)==InputLabelType(2) & x(:,1)>labels(K)));
        FN=length(x(x(:,2)==InputLabelType(1) & x(:,1)<=labels(K)));
        TN=length(x(x(:,2)==InputLabelType(2) & x(:,1)<=labels(K)));
    else
        TP=length(x(x(:,2)==InputLabelType(1) & x(:,1)<labels(K)));
        FP=length(x(x(:,2)==InputLabelType(2) & x(:,1)<labels(K)));
        FN=length(x(x(:,2)==InputLabelType(1) & x(:,1)>=labels(K)));
...
%     set(H,'Position',[4 402 560 420])
    ...
    if ubar > hbar
        Area = 1 - Area;
        title(sprintf('ROC curve (AUC=%0.4f) (Invert)',Area))
    else
