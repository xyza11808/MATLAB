...
PopuFieldSynchrony_all = cell(NumFolders,1);
for cfff = 1:NumFolders
    ...
    PopuFieldSynchrony_all{cfff} = cFieldSynIndex;
...
WTDataCoef = load(fullfile(WTDataPath,WTDataFn),'MultiSessCoefData');
WTSessData_Cell = WTDataCoef.MultiSessCoefData;

[TgDataFn,TgDataPath,~] = uigetfile('*tgDataSummary.mat','Select Tg data');
TgDataCoef = load(fullfile(TgDataPath,TgDataFn),'MultiSessCoefData');
...
WTEventIndexData_cell = WTDataEvent.EventSyncFrac;
...
TgEventIndexData_cell = TgDataEvent.EventSyncFrac;
...
WT_popuSynchronyVec = cell2mat(WT_popuSynchrony_All);
Tg_popuSynchronyVec = cell2mat(Tg_popuSynchrony_All);
...
%%
saveas(HHf,'p18 Sync Peak number and fraction compare plot');
saveas(HHf,'p18 Sync Peak number and fraction compare plot','png');
saveas(HHf,'p18 Sync Peak number and fraction compare plot','pdf');
