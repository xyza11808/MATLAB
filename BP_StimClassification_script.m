...
Data = Respdata;
OutPutData = trialFreqInds;
%%
TrainInds = randsample(size(Data,1),round(size(Data,1)*0.8));
TrainIndslogical = false(size(Data,1),1);
TrainIndslogical(TrainInds) = true;
TrainData = Data(TrainIndslogical,:);
TrainOutPutData = OutPutData(:,TrainIndslogical);
TestData = Data(~TrainIndslogical,:);
TestOutPut = OutPutData(:,~TrainIndslogical);
% each row will be trained once
nTrainTimes = size(TrainData,1);

%%
nHiddenLayer = 3; % 3 hidden layers
nHidNodesNum = [20,10,20];
nLearnRate = 0.2;
nInputNodes = size(TrainData,2); % input nodes
nOutputNodes = size(TrainOutPutData,1);
...
m = 1;
tTrainTime = tic;
for nTrainIter = 1 : nTrainTimes
    cTrainInput = TrainData(nTrainIter,:);
    InputData = cTrainInput(:);
    
    cOutput = TrainOutPutData(:,nTrainIter);
    OutputData = cOutput;
